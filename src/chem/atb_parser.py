...
def detect_fail_stage(result: Dict[str, Any]) -> Optional[str]:
    ...
    # Check NEB (support both old and new key names)
    if "NEB" not in result and "exciting_path_mean_volume" not in result:
...
def extract_features(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    gs = result["ground_state"]
    es = result["excited_state"]
    # Support both old and new key names for NEB
    neb = result.get("NEB") or result.get("exciting_path_mean_volume")
    ...
    # Excitation energy (if available in new format)
