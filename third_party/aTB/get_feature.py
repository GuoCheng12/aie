...
def calculate_structure_prop(args,type_calc):
    ...
    if not atoms:
        log.error(f"Failed to read structure file for {type_calc} calculation.")
        return None
...
def get_features_dict(aop, run_type, log, args):
    """
    ...
    features_name = args.properties.split(',')
    ...
    for func in features_name:
        if func == 'HOMO-LUMO':
            ...
            raise ValueError(f"Feature {func} not recognized.")
