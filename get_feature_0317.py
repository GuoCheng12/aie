...
def calculate_structure_prop(args,type_calc):
    ...
    max_dihedral = np.max(np.abs(dihedral_angles)) if dihedral_angles else 0.0
...
def get_transition_dipole_moment(text):
    """
    get transition dipole moment from text
    output: list of transition dipole moment
    The list index corresponds to the excited state order.

    """
    pattern = r"Ground to excited state transition electric dipole moments\(Au\):\s*(.*?)\s*Ground to excited state transition magnetic dipole moments\(Au\)"
    excited_array = re.findall(pattern, text, re.MULTILINE | re.DOTALL)
    excited_array = excited_array[-1].strip().splitlines()
    s1_transition_dipole_moment = excited_array[1].strip().split()    
    _dict = {}
    _dict['x'] = float(s1_transition_dipole_moment[2])
    _dict['y'] = float(s1_transition_dipole_moment[3])
    _dict['z'] = float(s1_transition_dipole_moment[4])
    _dict['dip'] = float(s1_transition_dipole_moment[5])
    _dict['osc'] = float(s1_transition_dipole_moment[6])
...
def get_features_dict(aop, run_type, log, args, *features_name):
    ...
    for func in features_name:
        if func == 'HOMO-LUMO':
            ...
            if run_type == 'excit':
                if get_excited_energy(aop) and get_transition_dipole_moment(aop):
                    features[func] = get_excited_energy(aop)[0]
                    features['transition_dipole_moment'] = get_transition_dipole_moment(aop)
                else:
...
if __name__ == '__main__':
    ...
    def extract_features(result: Dict[str, Any]):
        ...
        s1_dihedral_max = s1_struct.get("max_DA")
        ...
        delta_rap = abs(s1_rap) - abs(s0_rap) if s0_rap is not None and s1_rap is not None else None
        ...
        features = {
            ...
            "excitation_delta_rotational_constant_c" : s1_rc_c - s0_rc_c,
            "excitation_delta_rotational_constant_a" : s1_rc_a - s0_rc_a,
            "excitation_delta_rotational_constant_b" : s1_rc_b - s0_rc_b,
            "excitation_delta_rays_asymmetry_parameter" : delta_rap,

            "excitation_delta_dihedral": delta_dihedral,
            "excitation_delta_bonds" : delta_bonds,
            "excitation_delta_angles" : delta_angles,
            "excitation_delta_volume": delta_volume,
            "exciting_path_volume_delta": neb-s0_volume,

            "excitation_delta_gap": delta_gap,
    ...
    # target_dir = Path('/mnt/afs/250010045/5-AIE-code/cache/atb/AA')
    ...
    for json_file in status_files:
        try:
            ...
            if run_status == 'success':
                ...
                excited_arry = get_transition_dipole_moment(S1_aop)

                structure_S0 = get_features_dict(S0_aop, 'opt', log, args, *["structure"])
                structure_S1 = get_features_dict(S1_aop, 'excit', log, args, *["structure",'excited_energy'])
                print(structure_S1)
                ...
                results['excited_state']['excited_energy'] = structure_S1['excited_energy']
                results['excited_state']['transition_dipole_moment'] = structure_S1['transition_dipole_moment']
