...
3. If a **major/important lesson** is learned, add it to `.codex/AGENTS.md` under “Known gotchas / lessons”.
...
### How to talk to Codex (doc scoping)
When requesting work, explicitly reference the docs to read using the `@` syntax:
- Always include `@.codex/AGENTS.md @doc/roadmap.md @doc/process.md`
...
- Always follow the documentation protocol:
  ...
  - Add major lessons to `.codex/AGENTS.md`
...
1. Always run commands in the `aie` conda environment.
   - Required pre-step for every terminal session: `conda activate aie`.
2. Never run project commands with system/default Python.
   - Use `python`/`pip`/`pytest` only after `conda activate aie`.
3. If `aie` is unavailable, stop and report instead of running in another environment.
---
